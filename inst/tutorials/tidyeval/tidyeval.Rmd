...
My goal in the second edition of this tutorial is to build on the Tidyverse folks' practical advice.
...
More spoilers -- the goal of tidy evaluation is to help you, as a function-writer, be unambiguous about whether a function argument refers to the data frame or to the calling environment. For example, in the code:
...
filter(mtcarts, mpg > 20)
```

The `filter()` function does not care that `mpg`is defined in the calling environment; it *knows* that `mpg` refers to `mtcars$mpg`. 

At rstudio::conf(2020), Lionel introduced some new syntax that makes tide evaluation a little bit easier.
...
I think these videos provide an excellent introduction to tidy evaluation; this tutorial is built around them. 
...
If you had not read this section, would you think that `dplyr::group_by()` is a selection function or an action function?
  ...
  - Run the code, as is.
  - Modify the `group_by()` expression to read `group_by(species = glue("{species}oid"))`.
    Run the code again. What do you think?
...
Similarly, is `ggplot2::aes()` a selection function or an action function?
  ...
  - Run the code, as is.
  - Modify the `aes()` expression to read `aes(x = height / 2, y = mass)`.
    Run the code again. What do you think?
...
When working with tidy evaluation, keep in mind if you are working with and selection function or an action function.

Remember, Tidyverse's default is to assume that a varaible name refers to the data frame; if it does not exist there, it will look in the environment.
...
There's another reason to use the `all_of()` function:
  ...
  - Improve the function code by using `all_of()`.
...
The idea is the same as when we made the string-based functions: we need to be unambiguous about the context of names -- data-frame or environment. 
...
The `{{}}` operator (the Tidyverse team calls it "curly curly") is interpreted in tidy evaluation as:
...
Let's start again with the string-varaible function, and change it to a bare-name function:
...
If you tried `bare_filter(mtcars, cyl, 6)`, it would work fine.
  ...
  - In the `bare_filter` function above, rewrite the filtering expression as `{{ var }} == !!name`.
    Does it work now?
