...
```{r filter-str-function eval=FALSE}
df <- starwars
var_name <- "birth_date"
value <- 19
...
If you are writing a package function, you will also have to import the __rlang__ function you use, like `UQ` and `sym`. As of the writing of this tutorial, the tidyverse folks are working on a new package [__usethis__](https://github.com/r-lib/usethis), that will make it very easy for you to use tidyeval functions in your package-functions, _and_ pass R CMD CHECK. The `use_tidyeval()` function is not on CRAN yet (2017-11-01), but imagine it will be there soon.

```{r}
filter_quo_value <- function(.data, var_quo, value) {
  filter(.data, UQ(var_quo) == UQ(value))  
}
...
filter_sym_value <- function(.data, sym, value) {
  ...
  filter(.data, UQ(sym) == UQ(value))  
}

filter_sym_value(starwars, birth_year, 19)

filter_sym_value(mtcars, cyl, 6)
```


## More stuff

This would be a good place to make a survey of other tidyeval resources.

Thankfully, Mara Averick has already done this with her [tidyeval resource roundup](https://maraaverick.rbind.io/2017/08/tidyeval-resource-roundup/), so the best thing I can do here is to point you that-a-way.
