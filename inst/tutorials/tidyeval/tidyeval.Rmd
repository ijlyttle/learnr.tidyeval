...
Spoiler alert -- you need to use tidy evaluation only if:
  ...
  - these functions use bare column-names as arguments, e.g. `dplyr::select(mtcars, mpg)`.
...
In the next few sections, we will discuss the principles and concepts of tidy evaluation, then show how you can use these to call functions that use tidy evaluation (like those in dplyr). Finally, we will discuss how you can write functions that use the tidy evaluation framework.
...
Indeed, a quosure is an expression that carries along with it the context where it was created. If we think of an expression as a set of instructions, we can think of a quosure as a set of instructions bundled with a set of materials. We will look at quosures more explicitly later in this tutorial.
...
Perhaps we can interpret "data first!" another way: when a tidyverse function evaluates an expression, it looks in the _data first_, then in the calling context. We can work confidently with tidyverse functions because they behave consistently according to a convention.

In the next sections, we will use tidy evaluation with dplyr functions: first calling them directly, then writing functions that call dplyr functions. 
...
In tidy evaluation, we compose expressions. In my (albeit limited) experience, I have come across three types of expressions; an expression can have more than one type:
